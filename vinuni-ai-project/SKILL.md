...
description: Build repeatable VinUni AI-in-action projects through guided intake, level selection, AI-agent-first architecture, lightweight backend/frontend wrappers, logging, docs, and evaluation. Use when starting a new practical AI course project, turning a rough idea into a structured AI agent app, creating project scaffolding, or asking a coding agent to interview the user before implementation.
---
...
First run the intake workflow, choose a project level, draft the required artifacts, ask for user confirmation, then implement only the approved scope.

Use advisory judgment during intake. If the user's answer is vague, risky, too broad, or mismatched with the selected level, explain why, offer practical options, recommend one default, and ask the user to choose. Never silently decide important product, architecture, or scope choices for the user.
...
5. Read [references/level-system.md](references/level-system.md) and recommend a level.
...
For explicit backend/frontend separation by project level, read [references/folder-structure-by-level.md](references/folder-structure-by-level.md) before scaffolding.
